...
export function getAriaControlsElements<T extends Element = Element>(
    ...
    if (!controlled || !controlled.length) throw new Error(`Required controlled elements not found in DOM: ${controller.getAttribute("aria-controls")}`);
    controlled.forEach((el) => {
        if (!(el instanceof ctor)) throw new Error(`Controlled element not of required type: ${{ controller, ctor, el }}`);
    })
