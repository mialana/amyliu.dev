/** pulled from 'slim-select/react' as typing + exports were unstable
 * re-porting to local is an approved workaround here:
 * https://github.com/brianvoe/slim-select/issues/513
 *
 * synced to upstream on v3.3.0
 */

import React, { useEffect, useRef, forwardRef, useImperativeHandle } from "react";
import SlimSelectCore from "slim-select";
import type { Config, Option, Optgroup } from "slim-select";
...
const SlimSelect = forwardRef<SlimSelectRef, SlimSelectProps>(
    ({ data, settings, events, cssClasses, value, onChange, children, multiple }, ref) => {
        ...
        useImperativeHandle(ref, () => ({ slimSelect: slimSelectRef.current }));
            ...
            if (!valueExists) {
                ...
                if (!Array.isArray(cleanValue)) {
                    ...
                    if (!hasPlaceholder) {
                        ...
                        const placeholderOption: Partial<Option> = { value: "", text: "", placeholder: true };
        ...
        useEffect(() => {
            ...
            const config: Config = { select: selectRef.current };
