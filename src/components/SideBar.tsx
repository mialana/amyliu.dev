...
export default function SideBar({
    ...
    return (
        <div
            className={`absolute w-screen transition-[max-width] duration-1000 md:relative md:w-auto ${open ? "z-20 max-w-screen md:max-w-[40vw]" : "z-5 max-w-0"} bg-white ${category == "NAV" ? "left-0 md:rounded-r-sm" : "right-0 md:rounded-l-sm"} h-full overflow-x-visible pt-8 pb-4 shadow-lg`}
            onClick={() => {
        ...
        >
            <div
                ...
                className={`invisible absolute top-0 h-4 cursor-pointer ${category == "NAV" ? "bg-linear-to-l" : "bg-linear-to-r"} from-[#FEF1E3] to-[#CBD4D1] ${positionInfo["absolutePosition"]} w-full ${category === "NAV" ? "rounded-r-xs" : "rounded-l-xs"} z-30`}
                onClick={(event) => {
            ...
            >
                <svg
                    className={`${(category === "NAV") === open ? "rotate-180" : ""} ${open ? "" : "animate-pulse delay-1100"} text-neutral-500 transition-all duration-300 ${open ? "z-35" : "z-25"}`}
                    width={18}
                    height={18}
                    viewBox="0 0 24 24"
                    fill={`${open ? "#fff" : "#737373"}`}
                    stroke={`${open ? "#fff" : "#737373"}`}
                    strokeWidth="0.5"
                    strokeLinecap="round"
                >
                    <path
